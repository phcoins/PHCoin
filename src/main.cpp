...
uint256 hashGenesisBlock("0x00000fac8c8b7a42250545e87ee97bbd1dbec3225254a88e71fd0f21d2e20cd9");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PHCoin: starting difficulty is 1 / 2^12
...
static const int nHardForkOne = 30;
...
const string strMessageMagic = "PHCoin Signed Message:\n";
...
{
    // PHCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)
    {
       nSubsidy = 13500000 * COIN;
	}
	if(nHeight >= 200000) // 10 coins per block
	{
		nSubsidy = 10 * COIN;	
	}

    // Subsidy is cut in half every 135000 blocks, which will occur approximately every 4 months
    nSubsidy >>= (nHeight / 135000); // PHCoin: 135,000 blocks in ~4 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // PHCoin: 1 day
static const int64 nTargetSpacing = 60; // PHCoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PHCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 60; // PHCoin: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // PHCoin: 1 day
...
{
    // PHCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000fac8c8b7a42250545e87ee97bbd1dbec3225254a88e71fd0f21d2e20cd9");
    }
...
bool InitBlockIndex() {
    ...
    // vMerkleTree: b0019d92bc054f7418960c91e252e7d24c77719c7a30128c5f6a827c73095d2a 
		...
		const char* pszTimestamp = "A Different kind of a Coin - Philippines";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        printf("%s\n", block.hashMerkleRoot.ToString().c_str());
        assert(block.hashMerkleRoot == uint256("0x5690a36dd30f92554cdb801965c161e1c8c60326dc4735e09e76d2230e38080a"));
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc5, 0xbc, 0xdf }; // PHCoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PHCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PHCoinMiner : ProcessBlock, block not accepted");
    }
